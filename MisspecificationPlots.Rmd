...
```{r}
tmpData <- avgData %>% filter(driftFuncName == "b2")
...
ggplot(tmpData, aes(x=T))+
  ...
  xlab("Time horizon (T)")+
  ggtitle(TeX("\\textbf{Average exploration time for $b_2$}"))+
  facet_grid(pText ~ driftpText, scales="free")+
  theme_bw()+
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5))+
  theme(plot.title = element_text(face = "bold", size=13),
...
ggplot(avgData, aes(x=T, group=driftFuncName))+
  ...
  xlab("Time horizon (T)")+
  ggtitle("Difference in number of decisions made by optimal and data-driven")+
  scale_color_manual(values = c("b1" = "#66c2a5",
                                "b2" = "#fc8d62",
                                "b3" = "#8da0cb"),
                     labels=c(TeX("$b_1$"), TeX("$b_2$"), TeX("$b_3$")))+
