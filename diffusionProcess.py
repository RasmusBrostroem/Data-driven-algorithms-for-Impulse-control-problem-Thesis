    ...
    def invariant_density(self, x):
        def integral(x):
            ...
            if isinstance(x, Iterable):
                return np.array(list(map(partial(quad, f, 0, limit=250, epsabs = 1e-3), x)))[:, 0]
            return quad(f, 0, x, epsabs=1e-3, limit=250)[0]
    ...
    def xi(self, x):
        def inner_integral(y):
            if isinstance(y, Iterable):
                return np.array(list(map(partial(quad, self.invariant_density, -np.inf, epsabs=1e-3, limit=250), y)))[:, 0]
            return quad(self.invariant_density, -np.inf, y, epsabs=1e-3, limit=250)[0]
        
        f = lambda y: 1/(self.sigma(y)**2 * self.invariant_density(y)) * inner_integral(y)
        if isinstance(x, Iterable):
            return 2*np.array(list(map(partial(quad, f, 0, epsabs=1e-3, limit=250), x)))[:, 0]
        return 2*quad(f, 0, x, epsabs=1e-3, limit=250)[0]
