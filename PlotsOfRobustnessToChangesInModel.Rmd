...
# Interaction between power and zeroVal
```{r}
...
ggplot(PowerAndZeroValAvgData, aes(x=T, group=zeroVal))+
  ...
  facet_wrap(~power, nrow = 2, scales = "free")
```
Looks very similar but with different y-axis corresponding to the effect from the different powers.
...
```{r}
individualData <- filter(data, power == 2 & C == 0.5 & zeroVal == 0.99)

individualAvgData <- individualData %>% group_by(T) %>% summarise(avgRegret = mean(regret),
                                            .groups = "drop")

# ggplot(individualData, aes(x=as.factor(T), y=regret))+
#   geom_boxplot()

ggplot(individualAvgData, aes(x=T, y=avgRegret))+
  geom_line()
```
