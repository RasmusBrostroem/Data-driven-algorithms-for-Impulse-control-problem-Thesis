...
avgData$aText = paste0("Diffusion A = ", avgData$sigma_func_A)
```
...
```{r}
tmpData <- fittedValuesDF %>% filter(diffusionFunc == "sigma3")
...
ggplot(tmpData, aes(x=T))+
  ...
  xlab("Time horizon (T)")+
  ggtitle(TeX("\\textbf{Fitted average cumulative regret for $\\sigma_3$}"))+
...
```{r}
tmpData <- avgData %>% filter(diffusionFunc == "sigma2")
...
ggplot(avgData, aes(x=T, group=diffusionFunc))+
  ...
  xlab("Time horizon (T)")+
  ggtitle("Difference in number of decisions made by optimal and data-driven for different diffusion coefficients")+
  scale_color_manual(values = c("sigma1" = "#66c2a5",
                                "sigma2" = "#fc8d62",
                                "sigma3" = "#8da0cb"),
                     labels = c(TeX("$\\sigma_1$"), TeX("$\\sigma_2$"), TeX("$\\sigma_3$")))+
...
thresholds$aText = paste0("Diffusion A = ", thresholds$sigma_func_A)
...
ggplot(data=thresholds, aes(cut_number, diffToOptimal))+
  ...
  labs(color = "Drift Slope")+
  facet_wrap(pText ~ aText, ncol=3, scales="free_y")+
  theme_bw()+
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5))+
  theme(plot.title = element_text(face = "bold", size=13),
        axis.title.x = element_text(size=12),
