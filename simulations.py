...
def simulate_dataDriven_vs_optimal(Ts,
    ...
    run["AlgoParams"] = {
        ...
        "a": DataStrat.a,
        "M1": DataStrat.M1,
        "ST_form": inspect.getsource(ST_form)
    }
    ...
    for s in range(sims):
        ...
        for T in Ts:
            ...
            diffusionProcess.generate_noise(T, 0.01)
            dataReward, S_T, thresholds_and_Sts, DataStratNrDecisions = DataStrat.simulate(diffpros=diffusionProcess, T=T, dt=0.01, ST_form=ST_form)
...
if __name__ == "__main__":
    ...
    Cs = [1/2, 4]
    powers = [1, 5]
    ST_forms = [lambda t: t**(1/4), lambda t: t**(1/3), lambda t: t**(1/2), lambda t: t**(2/3), lambda t: t**(3/4), lambda t: 2*(np.sqrt(2*np.sqrt(t)+1) + np.sqrt(t) + 1)]
