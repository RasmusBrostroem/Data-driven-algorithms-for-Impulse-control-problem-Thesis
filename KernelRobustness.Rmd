...
ggplot(AvgData, aes(x=ST, group=kernelMethod))+
  ...
  facet_wrap(~CText, nrow = 2, scales = "free")+
...
AvgDataCdf$CText = paste0("Drift slope = ", AvgDataCdf$C)
```


```{r}
ggplot(AvgDataCdf, aes(x=ST, group=CText))+
  geom_line(aes(y=avgMiseCdf, color=CText))+
  ylab("Mean Integrated Squared Error (MISE)")+
  xlab("Exploration time")+
...
```{r}
ggplot(AvgDataCdf, aes(x=ST, group=CText))+
  geom_line(aes(y=sdMiseCdf, color=CText))+
...
ggplot(avgDataStrat, aes(x=T, group=kernelMethod))+
  geom_line(aes(y=avgRegret/T, color=kernelMethod))+
  labs(title = "Average CR per time unit for kernel methods",
       y = "Average cumulative regret per time unit",
       x = "Time horizon (T)",
       color = "Kernel Method")+
...
```{r}
ggplot(avgDataStrat, aes(x=T, group=kernelMethod))+
  geom_line(aes(y=sdRegret/T, color=kernelMethod))+
  labs(title = "Standard deviation of CR per time unit for kernel methods",
       y = "Standard deviation of cumulative regret per time unit",
       x = "Time horizon (T)",
       color = "Kernel Method")+
  facet_grid(pText ~ CText, scales = "free")
```
...
avgBandwidthData$CText = paste0("Drift slope = ", avgBandwidthData$C)
